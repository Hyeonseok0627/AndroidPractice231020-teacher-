...
class TestIntentActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val manager = getSystemService(INPUT_METHOD_SERVICE) as InputMethodManager //InputMethodManager로 형 변환
        binding.showEditBtn.setOnClickListener {
            //에디트 텍스트 뷰에 강제로 포커스 주기.
            binding.emailEdit.requestFocus()
            //매니저 이용해서, 소프트 키보드 보여주기 제어
...
//            intent.putExtra("password","1234")
        ...
        // 인텐트 필터를 이용해서 외부 앱에 접근하기.
        // 인텐트 : 시스템에 전달하는 메세지
        // 의 속성에 intent-filter : 무엇을 하는지를 나타내는 라벨링.
        // 시스템이 해당 필터의 요소를 보고서 해당 외부 앱을 연결 시켜줌.
